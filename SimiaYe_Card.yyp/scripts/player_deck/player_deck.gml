#macro DEFAULT_PLAYER_DECK [obj_battack, obj_battack, obj_battack]
...
function add_card_to_player_current_deck(card) {
	check_for_player_current_deck()
	array_push(player_current_deck, card)
}
...
function remove_card_from_player_current_deck(card) {
	check_for_player_current_deck()
	var card_index = array_get_index(player_current_deck, card)
	array_delete(player_current_deck, card_index, 1)
}
...
function check_for_player_current_deck() {
	if(!variable_instance_exists(id, "player_current_deck")) {
		check_for_player_full_deck()
		var num_cards_in_full_deck = array_length(global.player_full_deck)
		player_current_deck = array_create(num_cards_in_full_deck)
		array_copy(player_current_deck, 0, global.player_full_deck, 0, num_cards_in_full_deck)
	}
}