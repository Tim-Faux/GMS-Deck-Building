...
function draw_character_highlight() {
	...
	var sprite_center_x = xstart - sprite_xoffset + (sprite_width / 2)
	var sprite_center_y = ystart - sprite_yoffset + (sprite_width / 2)
	var max_radius = highlight_radius * image_xscale
	if(is_selected) {
		max_radius *= 3 / 4	
	}
	else {
...
function draw_arrow_overhead() {
	...
	draw_set_alpha(1)
	var arrow_movement = abs(sin(arrow_pos * image_yscale / SELECTABLE_CHARA_ARROW_SPEED) * 10)
	var sprite_top = ystart - sprite_yoffset
	
	var arrow_width = SELECTABLE_CHARA_ARROW_WIDTH * image_xscale
	var sprite_center = xstart - sprite_xoffset + (sprite_width / 2)
